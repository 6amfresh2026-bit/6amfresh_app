...
import { findZoneForPoint } from '../../shared/zoneServiceability.js';
...
export const updateDeliveryAvailability = async (userId, payload) => {
    ...
    // pick up orders they cannot realistically fulfil for that seller.
    if (validStatus === 'online' && partner.restaurantId) {
        const checkLat = hasFreshCoords ? lat : partner.lastLat;
        const checkLng = hasFreshCoords ? lng : partner.lastLng;
        if (!Number.isFinite(checkLat) || !Number.isFinite(checkLng)) {
            throw new ValidationError('Location is required to go online');
        }
        const restaurant = await FoodRestaurant.findById(partner.restaurantId).select('zoneId').lean();
        if (restaurant?.zoneId) {
            const zone = await findZoneForPoint(checkLat, checkLng);
            if (!zone || String(zone._id) !== String(restaurant.zoneId)) {
                throw new ValidationError('You can only go online while inside your seller\'s delivery zone');
            }
        }
    }
