 ...
 * (and tests) trigger the same logic the scheduled job runs, without
 * waiting on real calendar dates. Accepts an optional ISO `asOf`.
 */
export async function runProductSubscriptionSweep(req, res, next) {
    try {
        ...
        const { generateUpcomingOccurrences, placeDueSubscriptionOrders } = await import('../../user/services/productSubscription.service.js');
        const generation = await generateUpcomingOccurrences(asOf);
        const placement = await placeDueSubscriptionOrders(asOf);
        res.status(200).json({
            success: true,
            message: 'Subscription sweep completed',
            data: { asOf: asOf.toISOString(), generation, placement },
        });
    } catch (error) {
        next(error);
    }
...
/**
 * POST /offers/monthly/run — manually runs the monthly-offer rollover +
 * upcoming-offer notification sweep on demand. Lets ops (and tests) trigger
 * the same logic the scheduled job runs, without waiting on real calendar
 * dates. Accepts an optional ISO `asOf` in the body to simulate "as of" date.
 */
export async function runMonthlyOfferSweep(req, res, next) {
    try {
        const asOf = req.body?.asOf ? new Date(req.body.asOf) : new Date();
        if (Number.isNaN(asOf.getTime())) {
            return res.status(400).json({ success: false, message: 'Invalid asOf date' });
        }
        const renewal = await adminService.renewMonthlyOffers(asOf);
        const notifications = await adminService.notifyUpcomingMonthlyOffers(asOf);
        res.status(200).json({
            success: true,
            message: 'Monthly offer sweep completed',
            data: { asOf: asOf.toISOString(), renewal, notifications },
        });
    } catch (error) {
        next(error);
    }
