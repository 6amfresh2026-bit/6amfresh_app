...
import { sendNotificationToOwner, notifyOwnerSafely } from '../../../../core/notifications/firebase.service.js';
...
export async function createAdminOffer(body) {
    ...
    const doc = await FoodOffer.create({
        ...
        isMonthly: body.isMonthly ?? false,
        notifyDaysBeforeNextMonth: body.notifyDaysBeforeNextMonth ?? 23
    });
...
export async function expireExpiredOffers() {
    const now = new Date();
    // Monthly offers are rolled forward by renewMonthlyOffers() instead of
    // expiring — excluding them here stops this sweep fighting that one.
    await FoodOffer.updateMany(
        { status: 'active', endDate: { $lte: now }, isMonthly: { $ne: true } },
        { $set: { status: 'inactive' } }
    );
}

/**
 * Rolls every auto-renewing monthly offer whose window has ended forward to
 * the next calendar month, reactivating it and resetting its usage counters.
 * Idempotent: once rolled, endDate is in the future so it won't match again
 * until next month's sweep.
 */
export async function renewMonthlyOffers(now = new Date()) {
    const expired = await FoodOffer.find({
        isMonthly: true,
        endDate: { $lte: now },
    });
    ...
    for (const offer of expired) {
        // Roll forward from the offer's own expired window, not from "now" —
        // so an offer that missed a sweep (e.g. server was down) advances one
        // month at a time instead of skipping straight to the current month.
        const prevEnd = offer.endDate ? new Date(offer.endDate) : now;
        const nextStart = new Date(prevEnd.getFullYear(), prevEnd.getMonth() + 1, 1, 0, 0, 0, 0);
        const nextEnd = new Date(prevEnd.getFullYear(), prevEnd.getMonth() + 2, 0, 23, 59, 59, 999);
    ...
    if (renewedCount > 0) {
        logger.info(`renewMonthlyOffers: rolled forward ${renewedCount} monthly offer(s)`);
    }
    return { renewedCount };
}

/**
 * Notifies eligible customers about an upcoming monthly offer N days
 * (offer.notifyDaysBeforeNextMonth, default 23) before the next calendar
 * month starts. Scoped to customers who have previously ordered from the
 * offer's restaurant(s) for a restaurant-scoped offer; all customers for an
 * admin-wide ('all' scope) offer.
 */
export async function notifyUpcomingMonthlyOffers(now = new Date()) {
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const nextMonthStart = new Date(now.getFullYear(), now.getMonth() + 1, 1);
    const daysUntilNextMonth = Math.round((nextMonthStart.getTime() - today.getTime()) / 86400000);
    const targetMonthKey = `${nextMonthStart.getFullYear()}-${String(nextMonthStart.getMonth() + 1).padStart(2, '0')}`;

    const candidates = await FoodOffer.find({
        isMonthly: true,
        status: 'active',
        notifyDaysBeforeNextMonth: daysUntilNextMonth,
        lastNotifiedForMonth: { $ne: targetMonthKey },
    }).lean();
    ...
    for (const offer of candidates) {
        ...
        if (offer.restaurantScope === 'selected') {
            const restaurantIds = (offer.restaurantIds?.length ? offer.restaurantIds : [offer.restaurantId]).filter(Boolean);
            userIds = await FoodOrder.distinct('userId', { restaurantId: { $in: restaurantIds } });
        } else {
            userIds = await FoodUser.distinct('_id', {});
        }

        const restaurant = offer.restaurantId
            ? await FoodRestaurant.findById(offer.restaurantId).select('name').lean()
            : null;
        const title = 'Next month’s offer is coming 🎉';
        const message = restaurant?.name
            ? `${restaurant.name} is running "${offer.couponCode}" next month. Look out for it from ${targetMonthKey}!`
            : `Coupon "${offer.couponCode}" will be active again from ${targetMonthKey}. Don't miss it!`;
        ...
        for (const userId of userIds) {
            try {
                await FoodNotification.create({
                    ...
                    message,
                    category: 'offer',
                    source: 'MONTHLY_OFFER',
                    metadata: { offerId: offer._id, couponCode: offer.couponCode, targetMonthKey },
                });
                await notifyOwnerSafely(
                    { ownerType: 'USER', ownerId: userId },
                    { title, body: message, data: { type: 'monthly_offer', offerId: String(offer._id), couponCode: offer.couponCode } },
                );
                notifiedCustomers += 1;
            } catch (err) {
                logger.warn(`notifyUpcomingMonthlyOffers failed for user ${userId}: ${err?.message || err}`);
            }
        }

        await FoodOffer.updateOne({ _id: offer._id }, { $set: { lastNotifiedForMonth: targetMonthKey } });
        notifiedOffers += 1;
    }

    return { notifiedOffers, notifiedCustomers, daysUntilNextMonth, targetMonthKey };
}
